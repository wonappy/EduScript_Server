...
def test_docx_creation():
    """DOCX 생성 테스트"""
    try:
        test_text = """안녕하세요! 이것은 DOCX 테스트입니다.

첫 번째 문단입니다. 한글이 잘 나오는지 확인해봅시다.
...
마지막 문단입니다. DOCX 생성이 성공적으로 완료되기를 바랍니다!"""
        
        result = create_docx_from_text(test_text, "test.docx", "테스트문서")
        
        # 로컬에 저장해서 테스트
        with open("test_output.docx", "wb") as f:
            f.write(result.to_bytes())
        
        print("DOCX 테스트 성공! test_output.docx 파일을 확인해보세요.")
        return True
        
    except Exception as e:
        print(f"DOCX 테스트 실패: {e}")
        return False
